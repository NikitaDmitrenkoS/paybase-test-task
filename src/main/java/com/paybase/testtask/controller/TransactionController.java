...
public class TransactionController {
    ...
    @GetMapping("/{id}")
    @Operation(summary = "Get transaction details", description = "Returns details for a transaction.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Transaction retrieved"),
            @ApiResponse(responseCode = "404", description = "Transaction not found")
    })
    public TransactionDetailsResponse getById(
            @Parameter(description = "Transaction identifier", example = "10")
            @PathVariable Long id) {
        return TransactionDetailsResponse.from(service.getById(id));
    }
}
